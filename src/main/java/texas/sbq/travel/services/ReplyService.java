...
public class ReplyService implements IService{
	...
	public String selectHseq() {
	 ...
	 public List<Opinion> CommentsDB() {
		 Opinion Comments= null;
		 List<Opinion> CommentsList = new ArrayList<>();
		 for(int i = 0 ; i<commentsList().size();i++) {
			 Comments = new Opinion(commentsList().get(i),ratingList(),makeUserid(), selectHseq());
	  ...
	  return CommentsList;
		...
		public void insertCommentsDB() {
			 List<Opinion> cList = CommentsDB();
			 System.out.println("==7==");
			for(int i=0;i<cList.size();i++) {
				//hotelMapper.insert(null);
			}
